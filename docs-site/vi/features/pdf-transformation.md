...
Transform PDF chỉ tạo ra một file `text-only.md`. Không có `full.md` hay thư mục `images/` — AI Vision đọc trực tiếp từ các trang được render.
...
File PDF sau khi nén là file tạm — chỉ dùng để gửi lên AI và sẽ bị xóa sau khi transform xong.
...
- Transform PDF chỉ cho **output text** — không trích xuất hình ảnh
