// lib/transformers/docx.ts
// DOCX transformer: Pandoc CLI → raw.md + images/
...
import { convertDocxWithAI } from '@/lib/ai';
...
export async function convertDocx(
  ...
  const mode = options.mode || 'pandoc';
  ...
  if (mode === 'pandoc') {
    ...
    // AI Mode: chuyển DOCX sang format trung gian trước khi gửi Gemini
    // Gemini không hỗ trợ MIME DOCX → phải dùng text (md/html) hoặc PDF
    const fmt = (await getSetting('docx_intermediate_format')) || 'html'; // 'md' | 'html' | 'pdf'
    const tmpPath = path.join(safeOutputDir, `__tmp.${fmt === 'pdf' ? 'pdf' : fmt}`);

    try {
      if (fmt === 'pdf') {
        // Pandoc → PDF → gửi PDF lên Gemini via inlineData (PDF được hỗ trợ)
        await execFileAsync('pandoc', [
          safeFilePath,
          '--pdf-engine=xelatex',
          '-o', tmpPath,
        ]);
        const { convertPdfWithAI } = await import('@/lib/ai/gemini');
        const aiMarkdown = await convertPdfWithAI(tmpPath);
        await fs.writeFile(rawMdPath, aiMarkdown, 'utf-8');
      } else {
        // Pandoc → MD hoặc HTML → gửi as text prompt
        const pandocFmt = fmt === 'html' ? 'html' : 'markdown';
        await execFileAsync('pandoc', [
          safeFilePath,
          '-t', pandocFmt,
          '--wrap=none',
        ...
        const textContent = await fs.readFile(tmpPath, 'utf-8');
        const aiMarkdown = await convertDocxWithAI(textContent);
        await fs.writeFile(rawMdPath, aiMarkdown, 'utf-8');
      }
    } finally {
