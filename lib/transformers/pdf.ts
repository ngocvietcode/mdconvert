// lib/transformers/pdf.ts
// PDF transformer: compress → Gemini → text-only.md
// Batch 20 trang nếu dài, tối đa 5 batch (100 trang)

import fs from 'fs/promises';
import path from 'path';
import { compressPdf } from '@/lib/compress/pdf';
import { convertPdfWithAI } from '@/lib/ai';
...
export async function convertPdf(
  ...
  if (pageCount <= PAGES_PER_BATCH || pageCount === 0) {
    ...
    markdown = parts.join('\n\n---\n\n');
    if (truncated) {
      markdown += '\n\n> **[Lưu ý]:** Tài liệu quá dài, chỉ transform 100 trang đầu.';
    }
  }
