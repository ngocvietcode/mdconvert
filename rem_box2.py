...
# Box 2 starts with `<div className="space-y-4">` and ends right before `</>`
# Let's target it specifically based on the map loop
pattern2 = r'<div className="space-y-4">\s*\{\(connectionsOverride \|\| endpoint\.connections \|\| \[\]\)\.map\(\(slug: string, idx: number\) => \{.*?Endpoint này xử lý Local hoặc qua các Local Processors nội bộ, không có External API Pipeline nào\.\s*</p>\s*\)\}\s*</div>'

merged_code = """<div className="space-y-4 mb-4">
...
text_new = re.sub(pattern2, merged_code, text, flags=re.DOTALL)
if text_new != text:
    print("Replaced successfully")
else:
    print("Failed to replace Box 2")
